...
class Crawler:    #一个版块定义一个Crawler
    ...
    def get_post(self, Parser):  #多线程获取文章列表，q为队列，存放页面数
        url = self.post_list_q.get(timeout=2)  #从队列中取一个链接
        html = load_page(url)
    ...
    def crawl_new_data(self):
        ...
        while isNew == 1:
            ...
            for post in post_list:  #对每条主帖进行判断
                ...
                if db_post != None:  #有记录说明该主帖已经被保存过 
                    ...
                    if update_time >= last_comment_time:  #若为普通主帖，则判断主帖和评论有无变更，点赞也可能把帖子
        ...
        """
        user_queue = queue.Queue()
        for user in self.user_list:  #将User放入队列中
            user_queue.put(user)
        thread_list = []
        for i in range(thread_num):  #多线程获取用户详情
            thread = detailThread('Thread'+str(i+1), parser, user_queue, db)   #
            thread.start()
            thread_list.append(thread)
        for thread in thread_list:
